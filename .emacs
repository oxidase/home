...
(advice-add 'gud-file-name :after-until #'(lambda (f)
  "Check if gud-lldb-directories contain some tramp prefixes."
  (let (value)
    (dolist (elt (gud-val 'directories) value)
      (when (file-exists-p (concat elt f))
        (setq value (concat elt f)))))))
