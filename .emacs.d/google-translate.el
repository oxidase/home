...
(defun google-translate-translate-text (source-language target-language text)
  "Translate TEXT from SOURCE-LANGUAGE to TARGET-LANGUAGE.
  Returns text with available translations of TEXT."
  (let* ((result "")
         (json
          (json-read-from-string
           (google-translate-insert-nulls
      ...
      (if dict
          ...
          (combine-and-quote-strings
            (loop for item across dict
                  collect (aref item 0)
                  append (loop for translation across (aref item 1) for index from 1
                               collect (format "%2d. %s" index translation)))
            "\n")
        ;; If DICT is nil, which occurs, for example, if we ask for a
        ;; translation of a whole phrase, then show the most probable
        ;; translation, found as the leftmost string in the JSON.
