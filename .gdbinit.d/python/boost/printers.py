...
try:
    class GDB_Value_Wrapper(gdb.Value):
        ...
        def __init__(self, value):
            super(GDB_Value_Wrapper, self).__init__()
            self.__dict__ = {}
except TypeError:
...
class BoostVariant:
    ...
    def to_string(self):
        ...
        types = list(map(lambda s: s.strip(), re.split(r', (?=(?:<[^>]*?(?: [^>]*)*))|, (?=[^>,]+(?:,|$))', m.group(1))))
        ...
        return '%s' % ('boost::none' if type == 'boost::blank' else data.dereference())
...
class Boost_Multi_Index:
    ...
    class ordered_iterator:
        ...
        def get_parent_ptr(node_ptr):
            return long(str(parse_and_eval('*((void**)' + str(node_ptr) + ')')), 16) # & (~1L) FIXME?
