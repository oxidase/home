...
class EigenMatrixPrinter:
    ...
    def to_string(self):
        variety = 'Array' if (str(self.val.type).startswith('Eigen::Array')) else 'Matrix'
        return "Eigen::%s<%s,%d,%d,%s> (data ptr: %s)" % (variety, self.innerType, self.rows, self.cols, "RowMajor" if self.rowMajor else  "ColMajor", self.data)

class EigenMatrixNumpyPrinter(EigenMatrixPrinter):
    def __init__(self, val):
        super(EigenMatrixNumpyPrinter, self).__init__(val)

    def children(self):
        return []

    def to_string(self):
        data = list((self.data + index).dereference() for index in range(self.rows * self.cols))
        if (str(self.innerType) == 'int'):
            data = [int(x) for x in data]
        else:
            data = [float(x) for x in data]
        matrix = np.array(data)
        matrix = matrix.reshape((self.rows, self.cols), order = 'C' if self.rowMajor else 'F')
        return super(EigenMatrixNumpyPrinter, self).to_string() + '\n' + np.array2string(matrix, separator=', ')
...
def build_pretty_printer():
    ...
    pp.add_printer('Eigen::Matrix', '^Eigen::Matrix<.*>$', EigenMatrixNumpyPrinter)
    pp.add_printer('Eigen::SparseMatrix', '^Eigen::SparseMatrix<.*>$', EigenSparseMatrixPrinter)
    pp.add_printer('Eigen::Array', '^Eigen::Array<.*>$', EigenMatrixNumpyPrinter)
    return pp
